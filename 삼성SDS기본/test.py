...
def Me(opp, turn, opp_prev, opp_last_pattern):
    ...
    for i in range(len(opp_last_pattern)):
        temp = list(set(opp_last_pattern[i]))
        if temp == [-1]:
            if i == 0: prev_game = 0
            else: prev_game = i-1
            break

    set_opp_last_pattern = list(set(opp_last_pattern[prev_game]))  # 사냥한 동물

    if set_opp_last_pattern == [-1]: # 첫 게임
        if opp_prev == DEER:
            result = DEER
        elif opp_prev == RABBIT:
            result = RABBIT
        elif opp_prev == SNAKE:
            result = SNAKE

#     상대의 전 게임 기록 분석
    elif len(set_opp_last_pattern) == 3: #골구로 냈을 때 >> 협동
        if opp_prev == DEER: result = DEER
        elif opp_prev == RABBIT: result = RABBIT
        elif opp_prev == SNAKE: result = SNAKE

    elif len(set_opp_last_pattern) == 2:
        if turn == 4: return DEER  # 교란작선

        if DEER not in set_opp_last_pattern: #사슴이 없으면 이놈들 배신자
            if turn%2==0:
                result = SNAKE
            else:
                result = random.randint(0,1) # 토끼와 사슴 랜덤으로
        elif RABBIT not in set_opp_last_pattern:
            if opp_prev == DEER:
                result = DEER
            elif opp_prev == SNAKE:
                result = SNAKE
        elif SNAKE not in set_opp_last_pattern:
            if opp_prev == DEER:
                result = DEER
            elif opp_prev == RABBIT:
                result = RABBIT

    elif len(set_opp_last_pattern) == 1:
        if set_opp_last_pattern == [DEER]: result = DEER
        elif set_opp_last_pattern == [RABBIT] or set_opp_last_pattern == [SNAKE]:
            result = SNAKE
